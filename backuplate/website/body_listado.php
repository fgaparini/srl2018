...
<div id="contenedor2">
  <div id="cont_full">
    ...
    <div align="left" id="#listado" itemscope itemtype="http://schema.org/ItemList" >
      <h1 itemprop="name"><?php echo $titulo_p; ?> en <?php echo $this->config->item('ciudadweb');; ?></h1>
      <?php if (count($rowD)>0 OR count($rowL)>0 ){ ?><span id="multiple" title="Envie una consulta a todos los <?php echo $titulo_p ?> en San Rafael - Mas facil, Mas rápido .."><a href="<?php  echo base_url()."website/multiple/emailmultiple/?tipoalojar=".$TAlojar."&nameA=".$titulo_p."&bu=".base_url(); ?>"  class="femail" onclick="_gaq.push(['_trackEvent','email','multiple','<?php echo $titulo_p; ?>']);"> Enviar consulta a <?php if($titulo_p=="Cabañas") {echo "todas las ".$titulo_p;} else {echo "todos los ".$titulo_p;} ?> </a></span><?php } ?>
      <?php  if (count($rowD)==0 & count($rowL)==0) { echo "<p>Lo Lamentamos pero no tenemos nngun registro de <b>".$titulo_p ." </b></p>" ;} ?>
      <p>Completa guía de <strong><?php echo $titulo_p; ?></strong> en <strong>San Rafael , Valle Grande, Cañón del Atuel</strong>, etc. para la mejor programación de su estadía en nuestra ciudad.</p>
    </div>
    ...
    <div id="dir_dest" class="">
      ...
      <div itemscope itemtype="http://schema.org/LodgingBusiness">
        ...
        <div class="dir_H" align="center">
          <h3> <a href="<?php echo base_url().'alojamiento/'.$var['Url'] ?>" title="Mas Detalle <?php echo $name_T." ".ucwords($var['Nombre']); ?> - Ver Ficha Informacion" onclick="_gaq.push(['_trackEvent','ficha','vista-desta','<?php echo $var['Nombre']; ?>']);">
