...
class Db_consultas extends CI_Model
{
 public function __construct()
    ...
    {
        $query = sprintf("select count(*) as cantidad from consultas c, propiedades p where p.Usuarios_ID_Usuarios='%s' AND c.Propiedades_ID_Propiedades=p.ID_Propiedades ", $id_user);
        $row   = $this->db_prop->query($query);
        $row   = $row->row_array();
        return $row['cantidad'];
    }
