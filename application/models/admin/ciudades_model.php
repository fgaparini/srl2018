...
class Ciudades_model extends CI_Model
{
   ...
   /* ------------------------  BUSCAR CIUDADES DE UNA PROVINCIA ------------------------ */

    function find_ciudad_prov($prov)
    {

        $query = sprintf("select Location,Name from ciudades Where Subdivision ='%s' AND Country='AR'", $prov);
        $rows = $this->db->query($query);
        $rows = $rows->result_array();
        return $rows;
    }
