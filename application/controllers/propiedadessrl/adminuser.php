...
class Adminuser extends CI_Controller {
	...
	public function index()
	{
		...
		// echo $a['PlanUsuarios_ID_PlanU'];
		// print_r($data['PlanUser']);
		// exit();
		$data['body']           ="home";
		$data['page']           ="home";
...
//DESACTIVA o ELIMINAR PROPIEDAD
...
if ($data['infoprop']['Coordenadas']!=NULL && $data['infoprop']['Direccion'] !="")
{
 $data['coordenadas2'] = $data['infoprop']['Coordenadas'];
  $data['direccion'] =$data['infoprop']['Direccion'].",San Rafael, Mendoza";
$data['coordenadas'] = explode(",", $data['infoprop']['Coordenadas']);
 $data['latitud'] = $data['coordenadas'][0];
 $data['longitud'] = $data['coordenadas'][1];
...
function addfotos($idprop)
	{
		...
		if ($id_user==$iduser['Usuarios_ID_Usuarios']) 
		{
				...
				if ($estado=="agregar") {
					$data['masfoto']="si";
					$data['cant_fotos']=  $this->propabm_model->images_count($idprop);
				}
				$data['idprop']      =$idprop;	
				$data['body']      ="add_fotos";
		$data['page']      ="nueva_prop";
...
if ($id_user==$iduser['Usuarios_ID_Usuarios']) 
		...
		$data['fotos']       = $this->propabm_model->listar_fotos($idprop);
			$data['cantifotos'] = count($data['fotos']);
...
// BORRAR IMAGENES CON DROPZONE
function dz_delete($idprop, $id_imgage) {
...
// $image_name=base_url().'upload/propiedades/'.$idprop. "_".$name_foto.".jpg";
// $thumb_chica=base_url().'upload/propiedades/thumb/'.$idprop. "_".$name_foto.".jpg";
// $thumb_grande=base_url().'upload/propiedades/thumbG/'.$idprop. "_".$name_foto."_g.jpg";	
if (!empty($_FILES)) {
	// 
   ...
   if (isset($id_foto) && $id_foto>0) {
     ...
     redirect(base_url() . "propiedadessrl/adminuser/listarfotos/".$idprop, 'refresh');
