...
class Reservar_fin extends CI_Controller
{
    ...
    {
        //Datos reserva
        $metodo_pago_post = $this->input->post("MP");
        $tipo_pago_post   = "A";

        $habitaciones_comas          = "0," .$this->input->post("id_hab"); ;
        $cantidad_habitaciones_comas = "0," . $this->input->post("cant_hab");
        ...
        //Datos Huesped
        $NombreHuesped        = $this->input->post("nombreH");//$this->input->post('NombreHuesped');
        $ApellidoHuesped      = $this->input->post("apellidoH"); //$this->input->post('ApellidoHuesped');
        $EmailHuesped         = $this->input->post("emailH"); //$this->input->post('EmailHuesped');
        $TelefonoFijo         = $this->input->post("telefonoH"); //$this->input->post('TelefonoFijo');
        $TelefonoCelular      = "11"; //$this->input->post('TelefonoCelular');
        $Ciudad               = "aa"; //$this->input->post('Ciudad');
        $Provincia            = "aaa"; //$this->input->post('Provincia');
        $ObservacionesHuesped = $this->input->post("comentarios");
        ...
        //alojamiento_id     
        $alojamiento_id = $this->input->post("id_alojar"); //$this->input->post('id_alojamiento');
        ...
        {
         if ($habitaciones_array[$i]>0 or $habitaciones_array[$i]!="") 
         {
        ...
        $cantidad_huesped = count($row_db_huesped);
        $ID_Huesped       = 0;
        ...
        if ($cantidad_huesped > 0 OR $cantidad_huesped !="")
        {
        ...
        for ($i = 1; $i <= $fe_count; $i++)
        {
            ...
            {
                if ($habitaciones_array[$z]>0 or $habitaciones_array[$z]!="") 
                {
        ...
        $data_mail['mensaje']            = "
        Señor huesped en breve le enviaremos el voucher con la confirmacion de la reserva

        ";
        $data_mail['tipo_Hotel']   = $tipo_Hotel;
        $data_mail['nombre_Hotel'] = $nombre_Hotel;
        ...
        $this->email->from('reservas@sanrafaellate.com.ar', 'Reserva en ' . $tipo_Hotel . ' ' . $nombre_Hotel);
        $this->email->subject('Voucher Reserva para ' . $tipo_Hotel . ' ' . $nombre_Hotel);
        $this->email->message($str_mail_huesped_pendiente);
        $this->email->send();

        $this->email->clear();
        $this->email->to("francogasparini@gmail.com"); //Cambiar el mail por el de contaco@sanrafaellate.com
        ...
        $datos['alojarmenu']=$this->fag->tiposalojar();
        // LOCALIDAD  ALOJAR
        //$data['row_Lo']=$this->dbfichas->localidad($result_Al['Localidad']);
        //AGENDA 
        $datos['row_A']=$this->fag->agenda();


        $datos['ingreso']=$fecha_ingreso;
        $datos['salida']=$fecha_salida;
        $datos['tipo_Hotel']   = $tipo_Hotel;
        $datos['nombre_Hotel'] = $nombre_Hotel;
        $datos['ApellidoHuesped']    = $ApellidoHuesped;
        $datos['NombreHuesped']      = $NombreHuesped;
        $datos['EmailHuesped']      = $EmailHuesped;
        ...
        $datos['body']="website/body_reserva_fin";
        $datos['title']= "Reserva Pendiente en ".$tipo_Hotel." ".$nombre_Hotel."| San Rafael Mendoza | San Rafael Late ";
        $datos['descripcion']="Reserva Pendiente en ".$tipo_Hotel." ".$nombre_Hotel." en San Rafael Mendoza - Paga en Hasta 12 Cuotas sin Interes - San Rafael Late - www.sanrafaellate.com";
        $datos['keywords']="Reserva, Online, reserve, hoteles, cabain, turismo, sin, interes";
        $this->load->view('templates/website/template_gral', $datos);    
